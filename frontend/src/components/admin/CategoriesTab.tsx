...
const CategoriesTab: React.FC<CategoriesTabProps> = ({ categories, setCategories }) => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      // For now, we'll send only the name since that's what the API expects
      const categoryData = {
        name: formData.name
      };
  ...
  return (
    <div className="space-y-6">
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
        <div>
          <h2 className="text-2xl font-bold text-cyan-400">Категории</h2>
          <p className="text-gray-400 text-sm mt-1">Управляйте категориями статей</p>
        </div>
        <button
      ...
      {isFormVisible && (
        <div className="bg-slate-800/80 backdrop-blur-sm rounded-xl shadow-xl border border-slate-700/50 p-6 animate-fadeIn">
          ...
          <form onSubmit={handleSubmit} className="space-y-6">
            ...
            <div>
              <label className="block text-gray-300 mb-2 font-medium">Иконка (Emoji)</label>
              <input
                type="text"
                value={formData.icon}
      ...
      <div className="bg-slate-800/80 backdrop-blur-sm rounded-xl shadow-xl border border-slate-700/50 overflow-hidden">
        <div className="overflow-x-auto">
          <table className="w-full">
            <thead className="bg-slate-700/50">
              <tr>
                <th className="text-left p-4 text-cyan-400 font-semibold">Иконка</th>
                <th className="text-left p-4 text-cyan-400 font-semibold">Название</th>
                <th className="text-left p-4 text-cyan-400 font-semibold">Описание</th>
                <th className="text-left p-4 text-cyan-400 font-semibold">Действия</th>
              </tr>
            </thead>
            <tbody>
              {categories.map((category) => (
                <tr key={category.id} className="border-b border-slate-700/50 hover:bg-slate-700/30 transition-colors">
                  <td className="p-4 text-2xl">{category.icon || '📁'}</td>
                  <td className="p-4">
                    <div className="font-medium text-white">{category.name}</div>
                    <div className="text-gray-400 text-sm mt-1">
                      {category.articles_count !== undefined ? `${category.articles_count} статей` : '0 статей'}
                    </div>
                  </td>
        ...
        {categories.length === 0 && (
          <div className="text-center p-12">
            ...
            <h3 className="text-lg font-medium text-gray-300 mb-1">Категорий пока нет</h3>
            <p className="text-gray-500">Добавьте первую категорию, чтобы начать</p>
            <button
              onClick={handleCreate}
