...
const CATEGORY_KEYWORDS: Record<NewsCategory, string[]> = {
  ...
  'кибербезопасность': ['кибер', 'интернет', 'онлайн', 'сайт', 'веб', 'сети', 'wifi', 'wifi-роутер', 'маршрутизатор'],
 ...
 */
export function categorizeArticle(article: NormalizedArticle): NewsCategory {
  const fullText = `${article.title} ${article.description || ''} ${article.content || ''}`.toLowerCase();
  
  // Check each category for matching keywords
  for (const [category, keywords] of Object.entries(CATEGORY_KEYWORDS)) {
    if (category === 'другое') continue; // Skip 'other' for now
 ...
 */
export function addCategories(articles: NormalizedArticle[]): (NormalizedArticle & { category: NewsCategory })[] {
